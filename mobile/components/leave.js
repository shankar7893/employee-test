...
class Leave extends React.Component {
  ...
  _handleDatePicked = async date => {
    if (date.getMonth() + 1 < 10) {
      this.setState({
        fromDate: `${date.getFullYear()}-0${date.getMonth() +
          1}-${date.getDate()}`
      });
    } else {
      this.setState({
  ...
  _handleToDatePicked = date => {
    if (date.getMonth() + 1 < 10) {
      this.setState({
        toDate: `${date.getFullYear()}-0${date.getMonth() +
          1}-${date.getDate()}`
      });
    } else {
      this.setState({
...
>>>>>>> c9967b53ee58aa259f66088e877c4ac8ba578066
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
          <View style={{ flex: 1, marginLeft: 20, marginRight: 20 }}>
            ...
            >
              <Text style={{ color: "#0c1d40" }}>{this.state.employeeId}</Text>
              <View style={{ flexDirection: "column", alignItems: "center" }}>
                <Text style={{ marginTop: 8, color: "#0c1d40" }}>
                  Leaves Left
                </Text>
                <Text style={{ color: "#1eab07", fontWeight: "bold" }}>
                  {this.state.leavesLeft}
                </Text>
              </View>
            </View>
              ...
              <Card>
                <CardItem>
                  <TouchableOpacity onPress={this._showFromDateTimePicker}>
                    ...
                    >
                      {this.state.fromDate === "" ? (
                        <Text style={{ color: "#cacaca" }}>From </Text>
                      ) : (
              ...
              <Card>
                <CardItem>
                  <TouchableOpacity onPress={this._showToDateTimePicker}>
                    ...
                    >
                      {this.state.toDate === "" ? (
                        <Text style={{ color: "#cacaca" }}>To </Text>
                      ) : (
            ...
            >
              <TouchableOpacity
                onPress={async () => {
                  axios
                    .post("https://pronteff.com/Prontee/api/leaverequest", {
                      company_id: await AsyncStorage.getItem("companyId"),
                      emp_id: await AsyncStorage.getItem("employeeId"),
                      dept_id: await AsyncStorage.getItem("departmentId"),
                      from_date: this.state.fromDate,
                    ...
                    .catch(error => {
                      if (
                        ...
                        if (
                          !(this.state.toDate == "" && this.state.reason == "")
                        ) {
                          if (
                            !(
                              this.state.reason == "" || this.state.toDate == ""
                            )
                          ) {
                            if (this.state.fromDate == "") {
                              alert("Please set from date and try again");
                            }
                          }
                        } else {
                          alert("Please set reason , to date and try again");
                        }
                        if (
                          !(
                            this.state.fromDate == "" && this.state.reason == ""
                          )
                        ) {
                          if (
                            !(
                            ...
                            if (this.state.toDate == "") {
                              alert("Please set to date and try again");
                            }
                          }
                        } else {
                          alert("Please set reason , from date and try again");
                        }
                        if (
                          !(
                          ...
                          if (
                            ...
                            if (this.state.reason == "") {
                              alert("Please set reason and try again");
                            }
                          }
                        } else {
                          alert("Please set from date , to date and try again");
                        }
                      } else {
                        alert("Fill all details");
                      }
