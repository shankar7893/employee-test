...
class Success extends React.Component {
  constructor(props) {
    ...
    this.state = {
      unique_id: "",
      curTime: `${new Date().getFullYear()}-${new Date().getMonth() +
        1}-${new Date().getDate()}`,
      timeStore: "",
  ...
  async componentDidMount() {
    ...
    const attendenceSubmitted = await AsyncStorage.getItem(
      "attendenceSubmitted"
    );
    ...
    if (this.state.verifyDate != checkDate) {
      axios
        .post("https://pronteff.com/Prontee/api/markattendance", {
          empid: employeeId,
